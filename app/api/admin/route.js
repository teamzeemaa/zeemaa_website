...
export async function POST(request) {
  ...
  if (action === 'login') {
    ...
    if (user) {
      ...
      addLog({ type: 'auth', event: 'login', username: user.username, ip: request.headers.get('x-forwarded-for') || 'unknown' });
  ...
  if (action === 'logout') {
    ...
    if (user) addLog({ type: 'auth', event: 'logout', username: user.username });
  ...
  if (action === 'toggleSection') {
    ...
    const updated = updateStore(s => { s.sections[key] = value; return s; });
    addLog({ type: 'content', event: `section_${value ? 'enabled' : 'disabled'}`, detail: key, username: authedUser.username });
    return NextResponse.json({ ok: true, sections: updated.sections });
  }

  if (action === 'togglePage') {
    const { key, value } = payload;
    const updated = updateStore(s => { s.pages[key] = value; return s; });
    addLog({ type: 'content', event: `page_${value ? 'enabled' : 'disabled'}`, detail: key, username: authedUser.username });
  ...
  if (action === 'setList') {
    ...
    const updated = updateStore(s => { s[key] = items; return s; });
    addLog({ type: 'content', event: 'list_updated', detail: key, username: authedUser.username });
    return NextResponse.json({ ok: true, data: updated[key] });
  }

  if (action === 'updateSite') {
    const updated = updateStore(s => { s.site = { ...s.site, ...payload }; return s; });
    addLog({ type: 'settings', event: 'site_updated', username: authedUser.username });
  ...
  if (action === 'addBlogPost') {
    ...
    const updated = updateStore(s => { s.blog = [post, ...(s.blog || [])]; return s; });
    addLog({ type: 'blog', event: 'post_created', detail: post.title, username: authedUser.username });
  ...
  if (action === 'updateBlogPost') {
    ...
    addLog({ type: 'blog', event: 'post_updated', detail: id, username: authedUser.username });
    return NextResponse.json({ ok: true });
  }

  if (action === 'deleteBlogPost') {
    const { id } = payload;
    updateStore(s => { s.blog = (s.blog || []).filter(p => p.id !== id); return s; });
    addLog({ type: 'blog', event: 'post_deleted', detail: id, username: authedUser.username });
    return NextResponse.json({ ok: true });
  }
  ...
  if (action === 'updateUser') {
    ...
    addLog({ type: 'users', event: 'user_updated', detail: id, username: authedUser.username });
    return NextResponse.json({ ok: true });
  }
  ...
  if (action === 'addUser') {
    ...
    const updated = updateStore(s => { s.users = [...(s.users || []), newUser]; return s; });
    addLog({ type: 'users', event: 'user_created', detail: newUser.username, username: authedUser.username });
  ...
  if (action === 'deleteUser') {
    ...
    updateStore(s => { s.users = s.users.filter(u => u.id !== id); return s; });
    addLog({ type: 'users', event: 'user_deleted', detail: id, username: authedUser.username });
    return NextResponse.json({ ok: true });
  }

  if (action === 'clearSubmissions') {
    if (authedUser.role !== 'master') return NextResponse.json({ ok: false, error: 'Not authorized' }, { status: 403 });
    updateStore(s => { s.submissions = []; return s; });
    return NextResponse.json({ ok: true });
  }

  if (action === 'clearLogs') {
    if (authedUser.role !== 'master') return NextResponse.json({ ok: false, error: 'Not authorized' }, { status: 403 });
    updateStore(s => { s.logs = []; return s; });
    return NextResponse.json({ ok: true });
  }
